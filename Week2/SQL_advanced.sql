...
HAVING SUM(oi.quantity * p.price) > (
  SELECT AVG(inner_tot.order_total)
  FROM (
    SELECT
      o2.store_id,
      o2.order_id,
  ...
  WHERE inner_tot.store_id = o.store_id
)
ORDER BY
  store_name,
  order_total DESC;
...
WITH product_revenue AS (
  SELECT
    ...
    ROW_NUMBER() OVER (
      ...
      ORDER BY SUM(oi.quantity * p.price) DESC, p.name
    ) AS rn
  FROM orders o
  JOIN stores s       ON o.store_id = s.store_id
  JOIN order_items oi ON o.order_id = oi.order_id
...
SELECT
  ...
  p.name AS product_name,
  i.on_hand,
  sbsp.total_units_sold,
  (sbsp.total_units_sold - i.on_hand) AS units_gap
FROM inventory i
JOIN sales_by_store_product sbsp
...
WHERE i.on_hand < sbsp.total_units_sold
